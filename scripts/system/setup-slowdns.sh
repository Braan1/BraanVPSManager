...
get_slowdns_domain() {
    ...
    rm -rf /root/nsdomain /etc/BraanVPSManager/slowdns-domain 2>/dev/null
    ...
    echo "$SUB_DOMAIN" > /etc/BraanVPSManager/slowdns-domain
...
setup_slowdns_files() {
    ...
    # Download SlowDNS server binary from BraanVPSManager repository
    wget -q -O /etc/slowdns/dns-server "https://raw.githubusercontent.com/braan1/BraanVPSManager/main/bin/dns-server" || {
    ...
    openssl req -new -x509 -key /etc/slowdns/server.key -out /etc/slowdns/server.pem -days 365 -subj "/C=US/ST=State/L=City/O=BraanVPSManager/CN=slowdns-server" || {
...
[Unit]
Description=Server SlowDNS By BraanVPSManager
Documentation=https://github.com/braan1/BraanVPSManager
...
if [ -f "/etc/BraanVPSManager/slowdns-domain" ]; then
    subdomain=$(cat /etc/BraanVPSManager/slowdns-domain)
...
main() {
    clear
    echo "=========================================="
    echo "  BraanVPSManager - SlowDNS Server Setup     "
    echo "=========================================="
    echo ""
